...
def switch_provider():
    ...
    if choice == "1":
        ...
        if not env_vars.get('OPENAI_API_KEY') or env_vars.get('OPENAI_API_KEY', '').startswith('your_'):
        ...
        print(f"\nOllama:")
        print(f"  - Base URL: {env_vars.get('OLLAMA_BASE_URL', 'http://localhost:11434')}")
        print(f"  - Model: {env_vars.get('OLLAMA_MODEL', 'llama3.2')}")
        print(f"\nOpenAI:")
        openai_key = env_vars.get('OPENAI_API_KEY', 'Not configured')
        if openai_key and not openai_key.startswith('your_'):
            print(f"  - API Key: {openai_key[:15]}...{openai_key[-4:] if len(openai_key) > 20 else ''}")
        else:
